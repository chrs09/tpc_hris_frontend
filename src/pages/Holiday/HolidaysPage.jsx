...
export default function HolidaysPage() {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      await fetchHolidays();
      alert(`Synced: ${result.created} added, ${result.updated} updated, ${result.skipped} skipped`);
    } catch (err) {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200 text-left">
              <th className="px-5 py-3 font-medium text-slate-500 text-xs uppercase tracking-wide">Date</th>
              <th className="px-5 py-3 font-medium text-slate-500 text-xs uppercase tracking-wide">Name</th>
              <th className="px-5 py-3 font-medium text-slate-500 text-xs uppercase tracking-wide">Type</th>
              <th className="px-5 py-3 font-medium text-slate-500 text-xs uppercase tracking-wide">Scope</th>
              <th className="px-5 py-3 font-medium text-slate-500 text-xs uppercase tracking-wide">Source</th>
          ...
          <tbody>
            {loading ? (
              ...
              holidays.map((h) => (
                ...
                >
                  <td className="px-5 py-3 text-slate-500 whitespace-nowrap">{h.holiday_date}</td>
                  <td className="px-5 py-3 text-slate-800 font-medium">{h.holiday_name}</td>
                  <td className="px-5 py-3">
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${typeStyles[h.holiday_type] || "bg-slate-100 text-slate-600"}`}>
                      {h.holiday_type.replace(/_/g, " ")}
                    </span>
                  </td>
                  <td className="px-5 py-3 text-slate-500 capitalize">
                    {h.scope}{h.city ? ` – ${h.city}` : ""}
                  </td>
                  <td className="px-5 py-3">
                    <span
                    ...
                    >
                      {h.source === "manual"
                        ? h.override_api ? "Manual · override" : "Manual"
                        : "API"}
                    </span>
                  </td>
